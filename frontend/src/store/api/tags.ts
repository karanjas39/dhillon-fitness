export const tag_yearly_stats = "yearly_stats";
export const tag_daily_stats = "daily_stats";
export const tag_admin_details = "adminDetails";
export const tag_customer_birthday_stats = "customer_birthday_stat";
export const tag_membership_stats = "membership_stats";
export const tag_all_customers = "all_customers";
export const tag_customer_detail = "customer_detail";
export const tag_customer_memberships = "customer_memberships";
export const tag_membership_detail = "membership_detail";
export const tag_all_memberships = "all_memberships";

export const tagTypes = [
  tag_yearly_stats,
  tag_daily_stats,
  tag_admin_details,
  tag_customer_birthday_stats,
  tag_membership_stats,
  tag_all_customers,
  tag_customer_detail,
  tag_customer_memberships,
  tag_membership_detail,
  tag_all_memberships,
] as const;
